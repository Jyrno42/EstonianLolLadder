...
class API
{
    ...
    public function Strap($arg)
    {
        ...
        if($action !== null && strlen($action) > 0)
        {
            foreach($this->Actions as $k => $v)
            {
                if($k == $action)
                {
                    ...
                    if($res !== null)
                    {
                        ...
                        if(($red = ApiHelper::GetParam("redirect", false, null, null)) !== null)
                        {
                            header("Location: $red");
                        }
                    }
            ...
            throw new Exception(sprintf(_("Action %s is not yet implemented."), $action));
        }
        ...
        throw new Exception(_("Action parameter is invalid."));
    }
